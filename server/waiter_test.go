...
func checkName(t *testing.T, req *pb.WaitRequest, op *lropb.Operation) {
	if !strings.HasPrefix(op.Name, "operations/google.showcase.v1alpha2.Echo/Wait/") {
		t.Errorf(
			"Wait() expected op.Name prefex 'operations/google.showcase.v1alpha2.Echo/Wait/', got: %s'",
			op.Name)
	}
	nameProto := &pb.WaitRequest{}
	encodedBytes := strings.TrimPrefix(
		op.Name,
		"operations/google.showcase.v1alpha2.Echo/Wait/")
