...
func RegisterHandlers(router *gmux.Router, backend *services.Backend) {
	...
	router.HandleFunc("/v1beta1/repeat/{info.f_string:[0-9a-zA-Z_% \"\\-]+}/{info.f_int32:[0-9a-zA-Z_% \"\\-]+}/{info.f_double:[0-9a-zA-Z_% \"\\-]+}/{info.f_bool:[0-9a-zA-Z_% \"\\-]+}:simplepath", rest.HandleRepeatDataSimplePath).Methods("GET")
